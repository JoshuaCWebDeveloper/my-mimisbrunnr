...
describe('IpfsService - IPNS Methods', () => {
    ...
    let mockKuboClient: {
        routing: { put: ReturnType<typeof vi.fn> };
        name: { resolve: ReturnType<typeof vi.fn> };
        dag: { put: ReturnType<typeof vi.fn> };
    };
    ...
    describe('publishToIpns', () => {
        ...
        beforeEach(async () => {
            ...
            mockPrivateKey = (await generateKeyPairFromSeed(
                'Ed25519',
                seed
        ...
        it('should successfully create and publish IPNS record', async () => {
            const ipnsName = await ipfsService.publishToIpns(
                testCid,
                mockPrivateKey
            );
        ...
        it('should create IPNS record with correct sequence number', async () => {
            ...
            const [, marshaledRecord] =
                mockKuboClient.routing.put.mock.calls[0];
        ...
        it('should handle Kubo RPC routing.put failure', async () => {
            mockKuboClient.routing.put.mockImplementation(async function* () {
                yield Promise.reject(new Error('Kubo routing.put failed'));
            });
        ...
        it('should derive IPNS name from private key public key', async () => {
            ...
            const expectedPeerId = peerIdFromPublicKey(
    ...
    describe('resolveIpns', () => {
        ...
        it('should handle IPNS resolution timeout', async () => {
            ...
            mockKuboClient.name.resolve.mockImplementation(async function* () {
                ...
                // Must yield something to be a valid generator
                yield undefined as never;
            });

            // Note: Actual timeout handling would require implementation in the method
            // This test documents the expected behavior - currently just completes with no results
        ...
        it('should handle invalid IPNS name format', async () => {
            ...
            await expect(
                ipfsService.resolveIpns(invalidIpnsName)
            ).rejects.toThrow();
        });
    ...
    describe('Ed25519 Key Format Compatibility', () => {
        ...
        it('should maintain key consistency across conversions', async () => {
            ...
            const { createIPNSRecord, marshalIPNSRecord, unmarshalIPNSRecord } =
                await import('ipns');
            ...
            expect(unmarshaled.value.toString()).toBe(
                `/ipfs/${testCid.toString()}`
            );
